use std::net;

use crate::errors::Error;
use crate::requests::{
    ResponseDetails, create_request_with_empty_body, request_http1_tls_response,
};

pub async fn request_address_as_body(service: &str) -> Result<ResponseDetails, Error> {
    let request = match create_request_with_empty_body(service) {
        Ok(req) => req,
        Err(e) => return Err(e),
    };

    match request_http1_tls_response(request).await {
        Ok(res) => Ok(res),
        Err(e) => return Err(e),
    }
}

pub async fn get_ip_address_from_body(response_json: &ResponseDetails) -> Result<String, Error> {
    if response_json.status_code != 200 {
        return Err(Error::Custom("response_json was not okay".to_string()));
    }

    // set address if request is successful
    let ip_address = match response_json.body.trim().parse::<net::IpAddr>() {
        Ok(ip) => ip.to_string(),
        Err(e) => return Err(Error::IpAddr(e.to_string())),
    };

    Ok(ip_address)
}
