...
pub async fn request_ip(mut results: UpdateIpResults, config: &Config) -> UpdateIpResults {
    ...
    let response_type = match get_ip_service(&results, &config) {
        ...
        _ => {
        	ip_service_result.errors.push("failed to find ip service".to_string());
        	return results;
      	}
    };
    ...
    match response_type {
    	_ => ip_service_result = address_as_body::request_address_as_response_body(ip_service_result).await,
    }
...
fn has_address_changed(results: &UpdateIpResults, ip_service_result: &IpServiceResult) -> bool {
    match (&results.ip_service_result.address, &ip_service_result.address) {
